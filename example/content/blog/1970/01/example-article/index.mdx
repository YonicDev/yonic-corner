...
Since 2.3.0, a full feed will now be generated with all content from the post.
...
Version 2.3.0 introduces a few additional meta and syntax extensions.
...
In the Modern version, the line numbering will respect the actual numbering after applying the "diff".
...
Set the `id` to the one from the desired video (in the `watch?v=<id>` part of the URL when viewed normally, that is, not Youtu.be nor YouTube Shorts or whatever Google comes up with playing videos). You can also use a different Invidious instance by setting the URL of said instance.

The Invidious API can fill in other metadata such as the title, description and duration, but in order to speed up and guarantee that all the metadata is available, you can also supply them manually.
